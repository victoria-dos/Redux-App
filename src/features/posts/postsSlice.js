...
const initialState = {
  ...
  pagination: { ...defaultSettings, count: 3 },
}
...
const postsSlice = createSlice({
  ...
  reducers: {
    ...
    postUpdated(state, action) {
      const currentPost = state.data.find((post) => post.id === action.payload.id);
      if (currentPost) {
...
export const { pageSet, postAdded, postDeleted, postUpdated } = postsSlice.actions;
